...
size_t WiFiClient::write(uint8_t b, bool last)
{
    return write(&b, 1, last);
}

size_t WiFiClient::write(const uint8_t *buf, size_t size, bool last)
{
    if (!_client || !size)
    {
        return 0;
    ...
    return _client->write(reinterpret_cast<const char*>(buf), size, last);
}

size_t WiFiClient::write_P(PGM_P buf, size_t size, bool last)
{
    if (!_client || !size)
    {
        return 0;
    ...
    while (buf != NULL && remaining_size > 0) {
        ...
        _client->write((const char*)chunkUnit, chunkUnitLen, last && remaining_size == 0);
    }
    return size;
}
